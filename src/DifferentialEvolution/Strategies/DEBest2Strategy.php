...
class DEBest2Strategy implements IStrategy
{
    ...
    public function calculate($individuals, $probCrossOver, IObjectiveFunction $objectiveFunction, $F1, $F2 = 0)
    {
        ...
        for ($j = 0; $j < $populationSize; ++$j) {
            ...
            $r4 = $individuals[rand(0, $populationSize - 1)];
            $r5 = $individuals[rand(0, $populationSize - 1)];
            ...
            for ($k = 0; $k < $individualSize; ++$k) {
                ...
                if (Math::getRandomValue() <= $probCrossOver || $k == $varInd) {
                    $trial[$k] = $rBest->get($k) + $F1 * ($r2->get($k) - $r3->get($k) + $r4->get($k) - $r5->get($k));
                } else {
